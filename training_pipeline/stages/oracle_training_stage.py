...
class OracleTrainingStage(PipelineStage):
    ...
    def validate_inputs(self, context: Dict[str, Any]) -> List[str]:
        ...
        if 'sampled_events' not in context:
            errors.append("Missing required input: sampled_events")
        elif not context['sampled_events']:
            errors.append("No events to train on")

        return errors

    def execute(self, context: Dict[str, Any]) -> StageResult:
        sampled_events = context['sampled_events']
    ...
    def _train_audio_oracle(self, events: List[Any]) -> tuple:
        ...
        oracle = trainer.train_incremental(events)
