...
- **Curious Child.wav**: My own composition, instrumental. Piano trio with Eple Trio (Andreas Ulvo, Sigurd Hole, myself on drums). Very complex rhythmically—Scandinavian jazz style, lyrical but rhythmically intricate. The quantizing challenge is real. 
...
When I was trying to extract "Cmaj7" from audio and teach the AI to learn "Cmaj7 follows Dm7," I was forcing human symbolic thinking onto machine perception. Wrong approach. Took me months to figure that out.

Here's what I realized: Wav2Vec 2.0, a neural network trained on massive amounts of audio, learns to perceive sound in a 768-dimensional feature space (Bujard et al., 2024). It's not extracting chord names or note names—it's learning perceptual patterns, spectral qualities, timbral relationships. That's closer to how I actually hear music than symbolic analysis. I mean, when I'm improvising I'm not thinking "Cmaj7," I'm thinking about the sound, the color, the tension level.
...
Chord names like "Cmaj7" are only for me, to understand what's happening. The machine never sees them in its decision-making. This is a single source of truth—the machine and I are both perceiving the same audio data, just at different levels of abstraction. At least that's the theory.

#### But Rhythm Needs Numbers

Wav2Vec captures pitch and timbre well, but rhythm is trickier. Onset timing, duration patterns, rhythmic complexity—these are better analyzed mathematically. Or at least that's what I thought when I started.
...
Honestly, I'm still not sure I fully understand the Barlow metric. Something about prime factorization of the ratio numerator and denominator? But it works—high complexity correlates with what feels rhythmically dissonant.
...
[Interactive Demo 1: Web-based visualization of dual perception in action - play audio, see Wav2Vec feature extraction happening in real-time (768D vectors plotted), Brandtsegg ratio analysis overlaid (rational number ratios appearing as notes are detected), unified event structure building. Ideally interactive: user can upload their own audio file to see how MusicHal would analyze it.]
...
This is like working memory in human cognition—at least that's the metaphor I'm using. When MusicHal needs context to make a decision ("what have I been playing recently?"), it queries this buffer. The `phrase_generator.py` does this constantly: "Get me the last 10 gesture tokens," "What's the average consonance over the last 30 seconds," "Is the melody trending upward or downward?"

The buffer also normalizes features. Barlow complexity might range from 1 to 50, consonance from 0 to 1, gesture tokens from 0 to 255. Everything gets normalized to comparable ranges so multi-parameter requests can weight them appropriately. Which works, mostly. Sometimes the normalization is off and the requests get biased toward one parameter.
...
1. Load Itzama.wav (183.7 seconds of me playing electronic beat music/pop)
...
Based on IRCAM's research into musical interaction modes (Thelle & Wærstad, 2023), extended with my own additions. Though honestly I'm not sure if my extensions are improvements or just feature creep.
...
Musically: tight imitation, echoing my phrases, staying close to my melodic contour and rhythmic patterns. Like a harmony player who doubles your line. When it works, it feels supportive. When it doesn't, it feels like an annoying parrot.
...
These aren't arbitrary numbers—though they probably look arbitrary. I tuned them over dozens of sessions, listening for when modes felt musically distinct, when SHADOW felt too rigid or COUPLE felt too chaotic. The values in the table are current best guesses, not optimal solutions. SHADOW's similarity 0.9 might need to be 0.85, or 0.95, depends on the training data and the session context. Still iterating.
...
**[Video Example 6: Mode transition demonstration: split screen with me playing piano (left) and Mode/Request viewport (right). SHADOW mode showing high similarity 0.9, then countdown reaches zero, mode switches to COUPLE (similarity drops to 0.1, temperature jumps to 1.8). Audio synchronized so you hear my playing adapt as the parameters change. Duration: 3 minutes]**
...
This is where the rubber meets the road. When MusicHal decides to play something, how does it choose? This is the trust layer—if I can't understand this, I can't trust the system. Or at least, that's the theory.

Traditional AudioOracle generation is simple: traverse the graph, pick the next state based on transition probabilities. But that's pure statistical continuation, no musical intelligence, no ability to say "I want something consonant here" or "match my rhythmic pattern." Just blindly following the most likely next state.
...
Enter **request masking**. Which took forever to get working properly.
...
No teacher signal, no loss function to minimize. Just: absorb the musical structure, encode it in the AudioOracle's graph, serialize it to JSON. Whether this actually captures "musical structure" or just statistical patterns in high-dimensional space—that's the ongoing question.

`Chandra_trainer.py` is 1800 lines of this process. Probably could be shorter, honestly.
...
Itzama.wav - 183.7 seconds of electronic beat music/pop I recorded. Mono, 44.1kHz. The system loads it, displays waveform statistics, estimates tempo (around 140 BPM but it varies—electronic music, not quite as rubato as jazz but still has groove variation).
...
**Georgia.wav** (~120s)  
"Georgia on My Mind" (Carmichael/Gorrell, 1930). Jazz standard ballad—I sing with Andreas Ulvo on piano. Just voice and piano, no other instruments. Slow, intimate, rubato phrasing.

Why: test if system handles vocal material, slow tempos, duo interaction. Very different from Itzama's electronic beat music feel.
...
My own composition, instrumental. With Eple Trio—Andreas Ulvo (piano), Sigurd Hole (bass), and myself (drums). Piano trio formation. Very complex rhythmically, at least when it comes to quantizing it. Scandinavian jazz style, lyrical but rhythmically intricate.

Why: test extreme rhythmic complexity, trio ensemble interaction, quantizing challenging timing patterns.
...
This is why the system has a "voice"—it's shaped by the specific training audio. MusicHal trained on Itzama (electronic beat music/pop) sounds different from MusicHal trained on Georgia (jazz ballad) or Curious Child (complex Scandinavian jazz). Same algorithm, different learned structure, completely different genre character.
...
The problem hit me hard about 6 months into development. One of those bugs that makes you question the entire architecture.
...
The AudioOracle learned: "C follows Cdim follows C follows Cdim..."—a rapid flickering that never happened musically. When it generated, it played back this false rapid chord progression. Sounded manic, wrong, completely unusable.

This is over-sampling. The system doesn't know that samples 1-10 are all the same sustained note, not 10 separate events. Which seems obvious in retrospect, but took me weeks to diagnose.
...
But I didn't want to use traditional chroma extraction—that's separate from Wav2Vec, creates a mismatch between what the machine perceives and what I label. Remember: single source of truth. Which I'm maybe being too dogmatic about, but it seemed important at the time.

Solution: train a neural network to decode chord names FROM Wav2Vec features. Same 768D features the machine uses, just interpreted differently. A translator, basically.
...
Sub-50ms latency is the threshold, at least in theory. Beyond that, the system starts to feel sluggish, responses come too late to feel like dialogue. It's like talking to someone on a satellite phone with a 2-second delay—technically communication, but the conversational flow breaks. I'm not entirely sure where that 50ms threshold comes from—it might be psychological, might be musical—but it's real.

`MusicHal_9000.py` is ~1500 lines of real-time pipeline. Way more complex than it probably needs to be.
...
Which is kind of what Waters meant by performance ecosystems, I think.

**Unified Decision Engine:** The UnifiedDecisionEngine combines harmonic and rhythmic decisions based on response modes (support, contrast, imitate, lead). Correlation patterns learned during training are now loaded and passed to the decision context, enabling the engine to make choices informed by the harmonic-rhythmic relationships it observed in training data. This cross-modal intelligence is working—at least it's not crashing. Still being refined, though. The infrastructure's complete, the musical results are promising but honestly I'm not sure if the correlations are actually making decisions better or just adding complexity.

**[Video Example 1: Screen recording of live session with decision transparency showing pattern matching in real-time]**

Every component has to complete its work within milliseconds. Wav2Vec feature extraction's the slowest (20-30ms), but we batch it with onset detection so it doesn't add latency. In theory.
...
**Initialization** takes 2-3 seconds. Load trained model JSON (AudioOracle graph structure, metadata). Initialize Wav2Vec model (95MB, loads onto GPU if available). Set up MIDI interface—rtmidi scans for available ports, usually finds my IAC Driver. Initialize audio input, select microphone, buffer size 1024 samples. Create log files for conversation, decisions, audio analysis.

Wait, actually that's not quite the order. The MIDI interface setup happens first, then audio input, then model loading. Does the order matter? Probably not, but I'm not entirely sure.
...
These are classical variation techniques—Bach, Coltrane, everyone uses them. Building them into the algorithm gives MusicHal a basic toolkit for thematic development. Whether it actually understands what it's doing—that's a different question entirely.
...
**[Video Example 5: Close-up of phrase memory viewport during session showing: motif being extracted at t=0:45, display updates at t=1:15 (recall probability increasing), recall triggers at t=1:38 with "transpose +7" variation, room audio synchronized to show you hearing and responding to the varied motif. Duration: 2 minutes]**

But even with that limitation, thematic recall changes the feel of performances. Before phrase memory: reactive, moment-to-moment, no long-range structure. After phrase memory: developmental, coherent over time, feels like MusicHal has intent. When I hear that transposed motif come back 53 seconds later, it's not coincidence—the viewport shows MusicHal deciding to recall it. That visual confirmation matters. It's like seeing the gears turn.

---

[Figure 3: Screenshot of multi-viewport visualization system during performance, labeled to show each component's function - pattern matching window, mode parameters, phrase memory, audio analysis, timeline. Helps readers understand what Section 22.5 is describing.]
...
If I'd trained on a different genre—say Georgia's slow jazz ballad or Curious Child's complex Scandinavian jazz rhythms—the ratio distribution would be completely different. Georgia would show more rubato, different phrasing, different relationship to pulse. Curious Child would show extreme rhythmic complexity that's hard to quantize cleanly. The learned vocabulary is genre-specific, which is what I want.
...
That's why MusicHal trained on Itzama sounds different from MusicHal trained on Georgia or Curious Child. Different genres (electronic beat music vs. jazz ballad vs. complex Scandinavian jazz), different pitch distributions, different rhythm ratios, different harmonic progressions—different musical vocabulary.
...
**The modes feel distinct.** When SHADOW is active, I can feel it—the responses are close, imitative, supportive. When it switches to COUPLE, the independence is audible. The parameters (similarity threshold, temperature, request weight) create genuinely different musical characters. Sometimes. On good days.

**Thematic recall creates coherence.** When a motif returns transposed or inverted 60 seconds later, it's surprising—not random, but unexpected in a musically meaningful way. This is the closest the system gets to feeling "intentional," though I'm putting quotes around that word for obvious reasons.

**Decision transparency builds trust.** I think. Seeing "pattern match: 85% to gesture_token 142" confirms the system is listening. This matters more than I expected—without it, I'd be constantly second-guessing whether responses were genuine or arbitrary. Though honestly sometimes I still second-guess even with the transparency. Old habits.
...
**Mode transitions are sometimes jarring.** Switching from tight SHADOW imitation to independent COUPLE can feel abrupt if it happens during a musical moment that needs continuity. The 30-90 second timer is arbitrary, not musically aware. Actually, let me be more specific—it's ALWAYS arbitrary. Sometimes it happens to align with musical phrase boundaries, sometimes it doesn't.

Possible solution: mode switching based on musical features (density, consonance) not just elapsed time. If I'm building tension, stay in current mode. If tension resolves, that's a natural transition point. But I haven't implemented this yet, and I'm not sure how to detect "tension building" algorithmically.

**Phrase memory doesn't consider harmony.** When a motif gets transposed or inverted, it sometimes lands on notes that clash with the current harmonic context. The variation is rhythmically and melodically coherent but harmonically awkward. I mean, sometimes it works—accidental dissonance that resolves nicely. But more often it just sounds wrong.

Possible solution: harmonic-aware variations. Before applying transpose/invert, check if the result fits the current context (consonance level, gesture token relationships). Adjust the variation to maintain harmonic coherence. The infrastructure for this exists (gesture token relationships, consonance tracking), just haven't wired it together yet.
...
When I play with MusicHal, the musical output isn't "me" or "the algorithm"—it's the interaction. Patterns emerge from the feedback loop: I play something, MusicHal responds, I respond to the response, MusicHal responds to that, musical structure emerges from the coupling. At least, that's what happens on good days. Sometimes it just feels like two unrelated streams.

In this frame, whether MusicHal has "genuine" agency is the wrong question. The collective system has agency—the ability to produce musical outcomes neither part could create independently. That's what matters. Maybe.
...
This is the position I'm leaning toward. Not because it resolves the philosophical problem (it doesn't), but because it reframes the question in a way that's more productive for artistic research. Less "what is consciousness?" and more "what enables musical partnership?"
...
- `--enable-live-training`: Allow AudioOracle to learn from live input (see below)

#### Live Training vs. Pure Generation

By default, MusicHal generates purely from the pre-trained model without updating it during performance. This preserves the trained musical vocabulary—if you trained on Itzama's electronic beat music, MusicHal keeps that character throughout the session. The AudioOracle stays frozen; only your input and MusicHal's responses flow through the system.

With `--enable-live-training`, the system continuously learns from your playing, adding new patterns to the AudioOracle graph. The trained vocabulary remains accessible via suffix links, but as live patterns accumulate, generation may increasingly favor recent material. Within ~10 minutes of playing (~200 onsets), your live session starts to dominate the learned vocabulary.

Both approaches are valid:

- **Without live training** (default): Preserves trained character, more consistent responses, trained model defines the "voice"
- **With live training**: Adapts to current session, potentially more responsive, but shifts away from original training vocabulary

I usually run without live training. I want MusicHal to have a stable musical identity shaped by specific training data, not to gradually become a mirror of whatever I'm playing right now. But if you're doing long-term improvisation sessions where you want the system to evolve with you over hours, `--enable-live-training` makes sense.
...
I'm not sure the visualization actually solves anything. But it reveals things.

The `--visualize` flag opens five windows—viewports, I guess—showing different layers of what's happening inside MusicHal. Pattern matching scores, behavior mode parameters, phrase memory recalls, audio analysis, timeline of events. Updates 10 times per second, arranged in an automatic grid that tries to use the available screen space intelligently.

Does this help with trust? Maybe. The Chinese Room might be manipulating symbols, but at least I can watch which symbols it's manipulating and why. That's something.

When I play with a human musician, I trust they're listening because I can see them react—head nod, eye contact, body language that signals "I heard that, I'm responding." With a machine, there's no body language. Decision transparency in the logs helps (Section 12), but logs are post-hoc. I want to see it happening while I'm playing.

So I built this. Whether it actually works—whether seeing the mechanism builds trust—I'm honestly not sure yet.

#### What The Viewports Show

**Pattern Matching** shows the AudioOracle doing its thing—current gesture token, pattern match score (0-100%), which state in the graph it's at. When I play a phrase similar to something from training and the match score jumps from 45% to 87%, I can see it recognizing the pattern. That's verification, I guess. The pattern isn't random—it's genuinely matching learned structure. Or at least matching something numerical about the feature vectors.

**Request Parameters** makes the behavior mode system visible. Current mode (SHADOW, MIRROR, COUPLE, etc.), countdown timer until it switches, all the parameters for this mode (similarity_threshold, temperature, response_delay). The personality in numbers.

When the mode switches from SHADOW (similarity 0.95, temp 0.3) to COUPLE (similarity 0.05, temp 1.8, delay 3.0s), I don't just hear the relationship change—I see the exact parameter shifts causing it. Does knowing the numbers help? Sometimes. Sometimes it just feels like watching a dashboard while trying to have a musical conversation, which is kind of the opposite of flow state.

**Phrase Memory** shows stored motifs (first 5 notes as MIDI numbers), recall probability increasing over time, when variations trigger (transpose +7, invert, retrograde, whatever). A motif stored at t=0:45, recalled at t=1:38 transposed, then again at t=2:52 inverted—that's not random generation, that's thematic development. In theory. When it works.

**Audio Analysis** displays what the machine hears right now. Waveform (raw audio, 1024 samples), onset markers (visual spike when note detected), rhythm ratio from the Brandtsegg analyzer, Barlow complexity values. 

This one's actually useful. I can see the dual perception happening—when I play a dotted rhythm (3:2 ratio), the viewport shows "3:2". Complex polyrhythmic figure, Barlow complexity jumps to 8.5. Simple quarter notes, complexity drops to 0.0. It's working. At least the analysis part is working, whether MusicHal actually uses this information effectively is a different question.

**Performance Timeline** is mostly useless, honestly. It's supposed to show a scrolling event log—human input (green), AI responses (blue), mode changes (red), thematic recalls (yellow). The temporal overview of the whole session. But when I'm actually playing, I never look at it. Too much information, too fast. Maybe it's useful for post-performance analysis? Haven't tried yet.

#### What This Might Mean (Waters Again)

Waters (2007) talks about distributed cognition in performance ecosystems—how musical meaning isn't located in individual agents but emerges from the whole coupled system. The visualization makes this visible. I think that's what he meant, anyway.

I can watch the feedback loops happening: My phrase enters through the waveform display → gets analyzed into gesture token 142 and rhythm ratio 3:2 → triggers pattern matching (score jumps to 85%) → mode parameters shape the response (SHADOW: high similarity, low temperature) → phrase memory checks for recall (probability 72%) → generated phrase appears in timeline → I hear it → body responds with next phrase → cycle repeats.

The cognition isn't "in me" or "in MusicHal." It's in the circulation of information through this loop. The viewports show that circulation—make the ecosystem's structure visible rather than abstract. Whether that's actually useful or just conceptually interesting, I don't know yet.

#### Technical Bits

Built with PyQt5, five separate windows that auto-arrange in a grid. The audio processing thread emits Qt signals when state changes—these cross the thread boundary using something called QueuedConnection, which is apparently critical for thread safety. Ensures GUI updates don't block audio processing, which would be disastrous for latency.

Updates at 10Hz (every 100ms) regardless of audio processing rate (23ms chunks). Fast enough to feel real-time, slow enough that rendering doesn't kill performance. In theory.

#### Does Seeing Help?

When I see the pattern match score jump from 45% to 87% right as I play a phrase similar to something from the training data, that's verification. The machine IS using learned patterns, not generating randomly. Or at least, the numbers suggest it is. Whether "matching feature vectors in high-dimensional space" is the same thing as "listening to the phrase" philosophically—that's still unclear.

When the behavior mode switches from SHADOW (high similarity 0.9) to COUPLE (low similarity 0.1, long delay 3s, high temperature 1.8), and I feel the musical relationship change, the visualization confirms it's not my imagination—the parameters actually shifted. The personality isn't metaphorical, it's measurable. Which is either reassuring or disturbing depending on how you think about musical agency.

When a motif stored 60 seconds ago suddenly appears in the phrase memory viewport with "recall triggered: transpose +7," and I hear that transposed motif come back—yeah, okay, that helps. I know the thematic development isn't coincidence.

But seeing the mechanism doesn't automatically make the mechanism musically good. I can watch all five viewports and still feel like the responses are musically awkward, that the pattern matching found a technically similar phrase but emotionally wrong context. The visualization shows that something is happening, but it can't tell me if that something is artistically worthwhile.

What it does—what it might do, anyway—is eliminate one source of distrust: the worry that nothing meaningful is happening inside, that it's all arbitrary. Now I know something structured is happening. Whether that structure produces musical partnership, whether I can actually enter flow state while glancing at five windows of scrolling data—those are still open questions.

I'm not convinced yet. But I'm less uncertain than before.
...
- My own composition, instrumental
- Piano trio (Andreas Ulvo, Sigurd Hole, myself - drums)
- Very complex rhythmically—Scandinavian jazz style, lyrical but rhythmically intricate
- Focus on quantizing challenging timing patterns
...
### Appendix E: Notes on Multimedia Documentation (If I Ever Make It)

This document references a bunch of audio and video examples that don't exist yet. Here's what I'm trying to capture with each one, assuming I can figure out how to actually record them. Some of these are probably unnecessary, but writing them down helps me think about what evidence the argument actually needs.

**Audio Example 1: Feedback loop thing (Section 3.5)** — Need to record about 60 seconds of me and MusicHal going back and forth, showing clear adaptation. My phrase, it responds, I adjust based on what I heard, it responds to that adjustment, etc. The point is to show Waters' ecosystem feedback idea isn't just theoretical—it's actually happening. Might be hard to isolate a clean example, most sessions are messier than that.

**Video Example 1: Multi-viewport performance (Section 11)** — 3-minute screen recording with all five viewports visible, plus room camera if I can figure out how to sync them. Should show pattern match scores changing, a mode transition happening (ideally around 1:45), thematic recall marker appearing. The point is trust layer made visible—you can see the correspondence between what I play and what the viewports show. Though honestly I'm not sure how to record screen + camera simultaneously without expensive software.

**Audio Example 2: Itzama excerpt (Section 14)** — 30 seconds from the training audio, first climax section around 0:45-1:15. With spectral analysis overlay if I can figure that out. Point is to show what the training data actually sounds like—not "modal jazz" in the abstract, but this specific recording with its specific characteristics. The melodic gesture at 0:52 that becomes gesture_token 142 in the trained model. Readers should be able to verify the claims I'm making about the source material.

**Audio Example 3: Mode transition in live session (Section 15)** — 2-minute excerpt showing SHADOW→COUPLE transition, ideally around t=5:30-7:30 in a session. Both parts isolated (my input, MusicHal's output) plus mixed. The point: modes create audibly different behaviors. SHADOW should sound imitative, COUPLE should sound independent. If this doesn't work—if the modes don't sound distinct—then the whole sticky behavior idea is questionable.

**Video Example 2: Split-screen performance (Section 15)** — 2 minutes, left side room view (me playing drums/vocals), right side the visualization viewports. Synchronized, hopefully. Should show moment-by-moment correspondence—my onset happens, onset marker appears in viewport, pattern match score updates, phrase memory stores the motif. Demonstrates that analysis is responsive to actual playing, not random or delayed. Main technical challenge: syncing the camera with screen recording without drift.

**Figure 1: Barlow complexity graph (Section 14)** — Plot Barlow complexity over the full Itzama performance, annotate the 6-phase arc structure. Should show intro at low complexity (~1.5), building section increasing to 3.0, climax peaks around 5.5, resolution declining back down. Point is to show the arc isn't something I'm imposing on the data—it's there in the rhythmic structure. If the graph doesn't show this pattern, then the performance arc analysis is probably bullshit.

**Figure 2: Pattern match distribution (Section 15)** — Histogram of pattern match scores over a 15-minute session, mode transitions marked. SHADOW mode should show scores clustered 75-95%, COUPLE mode should be more scattered (20-80%). If they look the same, modes aren't affecting generation strategy like I think they are.

**Audio Example 4: Thematic recall sequence (Section 17)** — 3 minutes showing original motif at t=0:45, transposed variation at t=1:38, inverted at t=2:52. Long-term coherence, not just short-term pattern matching. This is the closest MusicHal gets to seeming "intentional." Need to find a session where this actually happens cleanly—most of the time the recalls are less obvious.

**Video Example 5: Phrase memory viewport (Section 13)** — 2 minutes focused on just the phrase memory viewport during a session where thematic recall works. Motif gets stored, recall probability increases over time, variation triggers, you see "transpose +7" appear. Room audio synchronized so you can hear the motif return. Purpose: showing this isn't post-hoc analysis, the system's doing it in real-time. If I can get the synchronization working.

**Video Example 6: Mode transition (Section 6)** — 3 minutes, split screen. Me playing piano, Mode/Request viewport visible. SHADOW mode (similarity 0.9, temp 0.3), countdown ticking down, transition at ~1:30, COUPLE mode (similarity 0.1, temp 1.8, delay jumps to 3s), my playing adapts to the new relationship. Should be dramatic—if it's subtle, the mode system isn't working as intended.

#### Do I Really Need All This?

Probably not. Six video examples feels like overkill. But when I'm writing "MusicHal listens to rhythm" or "modes create distinct personalities," those are empirical claims. They should be verifiable, not just assertions.

Videos 1, 2, and 5 address "is it really listening?"—showing real-time responsiveness between my playing and the system's analysis. Audio 3 and Video 6 are for "do modes actually work?"—if SHADOW and COUPLE don't sound different, the whole sticky behavior concept is questionable. Audio 4 and Video 5 are about thematic development—is it real or am I imagining patterns?

The multimedia isn't about making the document prettier. It's about making it checkable. You can watch Video 2 and verify: yes, the onset marker appears when he plays, no there's no mysterious delay, the analysis is genuinely responsive. Or watch it and conclude: no, there's too much lag, this doesn't support his claims.

Whether anyone will actually produce these videos, whether they're worth the effort to record and sync and edit—I don't know. But writing down what they should show helps me understand what evidence the argument needs. If I can't specify what would demonstrate that "MusicHal listens to rhythm," maybe I don't actually know what that claim means.
