# Exaggerated Behavior Modes - Now More Distinct! 🎭
...
- **Temperature:** 0.3 (SHADOW) → 1.0 (MIRROR) → 1.8 (COUPLE)
- **Similarity:** 0.95 (SHADOW) → 0.7 (MIRROR) → 0.05 (COUPLE)
- **Response Delay:** 0.1s (SHADOW) → phrase_aware (MIRROR) → 3.0s (COUPLE)
...
### 🎨 CONTRAST (Intentional Difference)
...
```
🎭 Mode shift: SHADOW (will persist for 67s)
→ Expect: Quick, chatty, quiet responses in same register

[67 seconds later]
🎭 Mode shift: COUPLE (will persist for 45s)
→ Expect: Delayed, sparse, LOUD responses octave higher

[45 seconds later]
🎭 Mode shift: MIRROR (will persist for 52s)
→ Expect: Phrase-aware, balanced, same register
```
...
### Key Changes:
1. **Temperature Range:** 0.3 → 1.8 (was 0.7 → 1.3)
2. **Similarity Range:** 0.05 → 0.95 (was 0.1 → 0.9)
3. **Delay Range:** 0.0s → 3.0s (was 0.2s → 2.0s)
...
### Trade-offs:
- ✅ **Pro:** Modes are NOW unmistakably different
- ✅ **Pro:** Creates variety and surprise
- ✅ **Pro:** Better matches IRCAM research intent
- ⚠️ **Con:** Might be TOO extreme for some tastes (can be dialed back if needed)
- ⚠️ **Con:** Pitch offsets might clash harmonically (intentional!)
...
✅ **6 distinct modes** with extreme parameter differences  
✅ **Pitch offsets** create register separation (octaves, 5ths)  
✅ **Note density** creates sparse/dense feel  
✅ **Response delays** from instant (SHADOW) to 3 seconds (COUPLE)  
✅ **Volume differences** from background (SHADOW 60%) to equal partner (COUPLE 120%)  

**Result:** Each mode now feels like a different musical personality! 🎭
